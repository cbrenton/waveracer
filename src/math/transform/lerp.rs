...
impl Lerp {
    pub fn new(start: DVec3, end: DVec3, smoothed: bool) -> Self {
        Self {
            start,
            end,
            scale: 1.0,
    ...
    pub fn end_early(start: DVec3, end: DVec3, stop_time: f64, smoothed: bool) -> Self {
        Self {
            start,
            end,
            scale: 1.0 / stop_time,
    ...
    pub fn start_late(start: DVec3, end: DVec3, start_time: f64, smoothed: bool) -> Self {
        Self {
            start,
            end,
...
mod tests {
    ...
    fn test_scaled_t_returns_correct_t() {
        ...
        let late_l = Lerp::start_late(DVec3::ZERO, DVec3::ONE, 0.2, false);
    ...
    fn test_end_early_valid_t_returns_start_and_end_interpolated_by_scaled_t() {
        let l = Lerp::end_early(DVec3::ZERO, DVec3::ONE, 0.8, false);
        assert!(almost_eq(l.at(0.0).unwrap(), DVec3::ZERO));
    ...
    fn test_start_late_valid_t_returns_start_and_end_interpolated_by_t_starting_from_start_time() {
        let l = Lerp::start_late(DVec3::ZERO, DVec3::ONE, 0.2, false);
        assert!(almost_eq(l.at(0.0).unwrap(), DVec3::ZERO));
    ...
    #[test]
    fn test_scaled_t_smoothed_returns_correct_t() {
        let l = Lerp::new(DVec3::ZERO, DVec3::ONE, true);
        assert_relative_eq!(l.scaled_t(0.0), 0.0);
        ...
        let early_l = Lerp::end_early(DVec3::ZERO, DVec3::ONE, 0.8, true);
        assert_relative_eq!(early_l.scaled_t(0.0), 0.0);
        assert_relative_eq!(early_l.scaled_t(0.4), 0.5);
        assert_relative_eq!(early_l.scaled_t(0.8), 1.0);
        assert_relative_eq!(early_l.scaled_t(1.0), 1.0);

        let late_l = Lerp::start_late(DVec3::ZERO, DVec3::ONE, 0.2, true);
        assert_relative_eq!(late_l.scaled_t(0.0), 0.0);
        assert_relative_eq!(late_l.scaled_t(0.2), 0.0);
        assert_relative_eq!(late_l.scaled_t(0.6), 0.5);
        assert_relative_eq!(late_l.scaled_t(1.0), 1.0);
